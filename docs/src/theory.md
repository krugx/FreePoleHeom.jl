...
$$\begin{aligned}
\frac{d}{dt}\hat{\rho}_{\mathbf{m,n}} &= -i \left[ \hat{H}_S^{RWA}, \hat{\rho}_{\mathbf{m,n}} \right] - \sum_{j=1}^{2M} (m_j \tilde{\gamma}_j + n_j \tilde{\gamma}_j^*) \hat{\rho}_{\mathbf{m,n}}\\
&\quad -i \sum_{j=1}^{2M} \sqrt{(m_j+1)\tilde{d}_j} \; \hat{q}_j \hat{\rho}_{\mathbf{m}_j^+, \mathbf{n}}\\
&\quad +i \sum_{j=1}^{2M} \sqrt{(n_j+1)\tilde{d}_j^*} \; \hat{\rho}_{\mathbf{m}, \mathbf{n}_j^+} \hat{q}_j\\
